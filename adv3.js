
// 1.


const clients1 = ["Гилберт", "Сальваторе", "Пирс", "Соммерс", "Форбс", "Донован", "Беннет"];
const clients2 = ["Пирс", "Зальцман", "Сальваторе", "Майклсон"];

const [ gilbert,  ,pirs, sommers, forbs, donovan, bennett] = clients1;

const [ ,zalzman, salvatore, michaelson] = clients2;

const clients3 =[gilbert, pirs, sommers, forbs, donovan, bennett,zalzman, salvatore, michaelson];

console.log(clients3);


// 2.

const characters = [
    {
      name: "Елена",
      lastName: "Гилберт",
      age: 17, 
      gender: "woman",
      status: "human"
    },
    {
      name: "Кэролайн",
      lastName: "Форбс",
      age: 17,
      gender: "woman",
      status: "human"
    },
    {
      name: "Аларик",
      lastName: "Зальцман",
      age: 31,
      gender: "man",
      status: "human"
    },
    {
      name: "Дэймон",
      lastName: "Сальваторе",
      age: 156,
      gender: "man",
      status: "vampire"
    },
    {
      name: "Ребекка",
      lastName: "Майклсон",
      age: 1089,
      gender: "woman",
      status: "vempire"
    },
    {
      name: "Клаус",
      lastName: "Майклсон",
      age: 1093,
      gender: "man",
      status: "vampire"
    }
  ];

  charactersShortInfo = [];

  characters.forEach(({name,lastName,age})=>{charactersShortInfo.push({name,lastName,age})})

  console.log(charactersShortInfo);


  // 3.

  const user1 = {
    name: "John",
    years: 30,
   
  };


  const {name,years:age,isAdmin = 'isAdminfalse'} = user1;


  alert(age);
  alert(name);
  alert(isAdmin);


  // 4.

  const satoshi2020 = {
    name: 'Nick',
    surname: 'Sabo',
    age: 51,
    country: 'Japan',
    birth: '1979-08-21',
    location: {
      lat: 38.869422, 
      lng: 139.876632
    }
  }
  
  const satoshi2019 = {
    name: 'Dorian',
    surname: 'Nakamoto',
    age: 44,
    hidden: true,
    country: 'USA',
    wallet: '1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa',
    browser: 'Chrome'
  }
  
  const satoshi2018 = {
    name: 'Satoshi',
    surname: 'Nakamoto', 
    technology: 'Bitcoin',
    country: 'Japan',
    browser: 'Tor',
    birth: '1975-04-05'
  }
  
  

  const {technology} = satoshi2018;
   
  const {browser,wallet} = satoshi2019;

  const {name:satoshiName,surname,age:satoshiAge,country,birth,location:{lat,lng}} = satoshi2020;


const fullProfile ={
    name:satoshiName,
    surname,
    age:satoshiAge,
    technology,
    country,
    birth,
    browser,
    wallet,
    location:{
        lat,
        lng
    }



}

console.log(fullProfile);

// 5.


const books = [{
    name: 'Harry Potter',
    author: 'J.K. Rowling'
  }, {
    name: 'Lord of the rings',
    author: 'J.R.R. Tolkien'
  }, {
    name: 'The witcher',
    author: 'Andrzej Sapkowski'
  }];
  
  const bookToAdd = {
    name: 'Game of thrones',
    author: 'George R. R. Martin'
  }


 const  {name:bookName,author} = bookToAdd;

 const newBooks = [...books,{name:bookName,author}]

 console.log(newBooks);


// 6.

const employee = {
    name: 'Vitalii',
    surname: 'Klichko'
  }

const{name:employeeName,surname:employeeSurname,age:employeeAge = 45 ,salary = 25000} = employee;

const newEmployee ={name:employeeName,surname:employeeSurname,age:employeeAge ,salary }
 
console.log(newEmployee);

// 7.

const array = ['value', () => 'showValue'];

const [value,showValue] = array;

alert(value);
alert(showValue());  
